...
class PopulateMsgs():
    ...
    def set_GetCartesianPositionRqt(self, group_name, frame_id, position, joint_names, waypoints):
        ...
        cart_pos_rqst.link_name = 'link_tcp'
        ...
        cart_pos_rqst.cartesian_speed_limited_link = 'link_tcp'
        cart_pos_rqst.max_step = max_step
        return cart_pos_rqst
    
    def set_CartesianPoint(self, position):
        cp_msg = CartesianPoint()
        orientation = Quaternion(x=0.96791, y=-0.24773, z=0.017813, w=0.038285) # TODO: Change to xArm specific Orientation
    ...
    def set_MotionPlanMsgs(self, frame_id, joint_names, start_positions, end_positions):
        """
        ...
        motion_plan_msgs.request = self.set_MotionPlanRq(
            frame_id, joint_names, start_positions, end_positions)
        self.state = State.IS_DIFF

        motion_plan_msgs.planning_options = self.set_PlanningOptions(
            frame_id, joint_names, end_positions)
    ...
    def set_MotionPlanRq(self, frame_id, joint_names, start_positions, end_positions):
        """
        ...
        motion_plan_rq.start_state = self.set_RobotState(
            frame_id, joint_names, start_positions)
        motion_plan_rq.goal_constraints = self.set_Constraints(
            frame_id, end_positions)

        motion_plan_rq.pipeline_id = "ompl"
        motion_plan_rq.group_name = self.group_name
        motion_plan_rq.num_planning_attempts = 10
        motion_plan_rq.allowed_planning_time = 7.0
        motion_plan_rq.max_velocity_scaling_factor = 0.05
        motion_plan_rq.max_acceleration_scaling_factor = 0.25
    ...
    def set_JointState(self, frame_id, joint_names, position):
        ...
        print(f"[Pop_msgs] positions: {position}")
    ...
    def set_IKRequestMsgs(self, robot_state, avoid, pose_stamped, timeout):
        ...
        IK_request_msgs.ik_link_name = "link_tcp"
